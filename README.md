...
Steps Completed:
    ...
    Card Component: Designed a reusable card that accepts product details as props.
    Homepage Layout: Implemented a grid/flexbox layout to display multiple cards neatly.
    Dynamic Rendering: Used array mapping to dynamically render cards for each product.
...
Added a Cart schema to store products associated with the user. Each product contains a productId (referencing the Product model) and quantity. Endpoint for Adding Products to Cart:

Created a POST endpoint /api/cart/add to receive product details (e.g., product ID and quantity). The product details are stored in the cart associated with the user in the database.
